...
class Agent:
    ...
    """
    def __init__(self, action_space: gym.spaces.Discrete, observation_space: gym.spaces.Box):
        """
        Initialize the agent.
        """
        # Store action spaces, observation spaces, and their sizes
        self.action_space_size = action_space.n # env.action_space: Discrete(4), the valid action choices would be integers 0, 1, 2, and 3
        self.observation_space_size = observation_space.shape[0] # env.observation_space: 8-dimensional vector
        ...
        self.previous_observation = None # will be needed because of this learn function
    ...
    def act(self, observation: gym.spaces.Box) -> gym.spaces.Discrete:
        ...
        if np.random.rand() <= self.epsilon:
            ...
            with torch.no_grad(): # because we don't want the Q-network to be updated
                q_values = self.q_net(torch.tensor(observation)) # only returns q values for each state in the positions 0, 1, 2, 3
    ...
    def learn(
        ...
        """
        self.replay_memory.append(Experience(self.previous_observation, self.previous_action, reward, observation, terminated)) # let's look at the previous experiences
        self.number_of_samples += 1
        
        if len(self.replay_memory) >= 100: # only if enough experiences to form a minibatch
            
            # Sample a minibatch of experiences from the replay memory
            random.sample(self.replay_memory, 100)
            minibatch = random.sample(self.replay_memory, 100) # also does not sample with replacement
            observations, actions, rewards, next_observations, dones = zip(*minibatch) # [(observation_1, observation_2, ...), (action_1, action_2, ...), (reward_1, reward_2, ...), (next_observation_1, next_observation_2, ...), (done_1, done_2, ...)]
            observations = torch.Tensor(observations) # shape: (100, 8)
            actions = torch.unsqueeze(torch.Tensor(actions), dim=1).to(torch.int64) # weird formatting modification needed: a) only int64 is allowed and b) need to convert actions list into a tensor. shape: (100, 1)
            ...
            prediction_qvals = self.q_net(observations).gather(1, actions.to(torch.int64)) # Get predictions. weird error where only int64 is allowed. self.q_net(observations) shape: (100, 4), actions shape: (100, 1). gather() returns a tensor with the same shape as the indices tensor, where each value of the output tensor is taken from the input tensor at the corresponding index. shape: (100, 1)
            next_qvals = self.t_net(next_observations).max(1)[0].unsqueeze(1) # Compute Q-values for the next observations using target network
            target_qvals = rewards + 0.99 * next_qvals * (1 - dones) # Compute target / ideal Q-vals. Gamma = 0.99 here.
